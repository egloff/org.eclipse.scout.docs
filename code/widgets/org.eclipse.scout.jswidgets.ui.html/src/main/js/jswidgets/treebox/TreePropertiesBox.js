...
jswidgets.TreePropertiesBox.prototype._setTree = function(tree) {
  ...
  var expandOnDoubleClickField = this.widget('ExpandOnDoubleClickField');
  expandOnDoubleClickField.setValue(this.tree.expandOnDoubleClick);
  expandOnDoubleClickField.on('propertyChange', this._onExpandOnDoubleClickPropertyChange.bind(this));
...
jswidgets.TreePropertiesBox.prototype._onExpandOnDoubleClickPropertyChange = function(event) {
  if (event.propertyName === 'value') {
    this.tree.expandOnDoubleClick = event.newValue;
  }
};
